...
void post_DHT22data();                          // DHT22のアナログ電圧をサーバーに送信
...
//EC-5
uint16_t one_shot_voltage[NUM_OF_SAMPLES];
...
void post_PSVolServer()
{
  ...
  float voltage = sum / NUM_OF_SAMPLES * (R1 + R2) / R2 / 1000.0; //電池電圧を分圧してる計算っぽい
...
void post_EC5data()
{
  ...
  float EC5Voltage = sum / NUM_OF_SAMPLES / 1000.0;
...
void post_DHT22data()
{
  ...
  // Block until we are able to connect to the WiFi access point
  HTTPClient http;
  http.begin("http://mist-hospital.mydns.jp/kabayaki/db/dht22");
  http.addHeader("Content-Type", "application/json");
  struct tm timeInfo;
  char detecting_time[20];
  char address_str[16];
