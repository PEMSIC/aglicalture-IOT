...
=======
//210324 BOOTCOUNT_const中は，5分に一回起動，その後時刻00分と30分に起動
>>>>>>> 2e895f935db47acd2d5e4be7f8b1137397ecf514
...
#include <PubSubClient.h>  //MQTT通信のために必要   追加

<<<<<<< HEAD
const char MY_SSID[] = "PIX-MT100";   //PIX-MT100
const char MY_PASSWORD[] = "mistmist";  //mistmist
...
<<<<<<< HEAD
  pinMode(OPEN_DRAIN_PIN, OUTPUT_OPEN_DRAIN); // 温度センサのスイッチ回路をオープンドレインで使用

  digitalWrite(GRM_GATE, HIGH);    // 接地抵抗計のロードスイッチをON
  digitalWrite(OPEN_DRAIN_PIN, LOW); // DS28B20のスイッチをオン(オープンドレインON LOW出力)
=======
  pinMode(DS18B20_GATE, OUTPUT_OPEN_DRAIN); // 温度センサのスイッチ回路をオープンドレインで使用
...
<<<<<<< HEAD
  digitalWrite(OPEN_DRAIN_PIN, HIGH); // DS28B20のスイッチをオフ(オープンドレイン開放 ハイインピーダンス)
=======
...
=======
  ...
  char GRString[8];
  dtostrf(GroundResistance, 1, 2, GRString); 
  client.publish("esp32/GroundResistance", GRString);
...
<<<<<<< HEAD
  ...
  char GRString[8];
  dtostrf(GroundResistance, 1, 2, GRString); 
  client.publish("agriIoT/GroundResistance", GRString);
