...
//Field ID
#define FIELDID 1004 // 島原"Shimabara1"のフィールド番号
...
void post_DS18B20data()
{
  ...
  for (int i = 0; i < numberOfDevices; i++)
  {
    ...
    JSONencoder["addr"] = address_str;
    JSONencoder["DS18B20"] = tempC;
    JSONencoder["field"] = FIELDID;
    ...
    JSONencoder["detect"] = detecting_time;
    ...
    if (client.publish("shirasuCorp/agriIoT/DS18B20", JSONmessageBuffer) == true) {
    ...
    /*
    String requestBody;
    serializeJson(doc, requestBody);  //ColdFusion データを JSON (JavaScript Object Notation) 表現に変換
    */
...
if (client.publish("shirasuCorp/agriIoT/PSVol", JSONmessageBuffer) == true) {
    Serial.println("Success sending message");
  } else {
    Serial.println("Error sending message");
  }
...
void post_wifilevel()
{
  ...
  if (client.publish("shirasuCorp/agriIoT/wifilevel", JSONmessageBuffer) == true) {
    Serial.println("Success sending message");
    } else {
    Serial.println("Error sending message");
    }
...
void post_EC5data()
{
  ...
  if (client.publish("shirasuCorp/agriIoT/EC5", JSONmessageBuffer) == true) {
    Serial.println("Success sending message");
    } else {
    Serial.println("Error sending message");
  }
