...
# import module  
# import:モジュール(arduinoでいうライブラリ)などを利用するために使用
...
# polling function
def pulse_polling():	#def:関数を表す		パルスポーリング:パルスを検出する関数

    global count	#global:関数内でグローバル変数に値を代入する時に使用
    global prev_status	#上に同じ

    if prev_status == 0 and GPIO.input(4) == 1:	#前の状態が0かつGPIOの4番ピンが1のとき
        ...
        catchTime = str(d.hour) + ":" + str(d.minute) + ":" + str(d.second)                 #カウントした時間、分、秒
        ...
        mesg = catchTime
        f.write(str(mesg))							#fファイルに対してテキストを書き込む		
        os.system('./db_call.sh')	#unixコマンドをpython上で記述するためのモジュール	osをインポートする必要がある
        f.write("\n")			#fファイルを改行
        f.close()			#fファイルを閉じる
    prev_status = GPIO.input(4)     #GPIO04のピンの状態を更新

# record file per hour
def record():       #主にファイルに時間を記述する？

    global count
    tStamp = datetime.datetime.today()		#現在の日付と時間を取得
    sleepInterval = (59 - tStamp.minute) * 60 + (60 - tStamp.second)		#スリープの間隔
    ...
    recordFile = open("/var/log/raingage/"+fallCountPerHour, "a")		#ファイル操作	open(ファイル名, オプション, 文字エンコード)	a:ファイルへの追加	w:ファイルへの書き込み(上書きする)	x:ファイルの新規作成
    mesg = str(recordTime) + ", " + str(count)
    recordFile.write(str(mesg))							#ファイルに対してテキストを書き込む
    recordFile.write("\n")							#改行
    recordFile.close()								#ファイルを閉じる
    count = 0									#カウントをリセット？
...
def main():
    ...
    prev_status = GPIO.input(4)		#GPIOの4番ピンの状態をprev_statusに代入
    ...
    while True:						#ループ		条件式がTrueなので、無限ループ
        pulse_polling()					        #pulse_polling()関数の呼び出し      下の処理と合わせると、0.01秒ごとに呼び出されている
